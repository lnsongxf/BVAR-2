...
p = 6;               % Number of lags on dependent variables
...
nburn = 20000;         % Draws to discard (burn-in)
...
if forecasting==1
    ...
    elseif forecast_method==1   % Iterated forecasts
        Y1 = Yraw;
        Y2 = Yraw;
    else % check if the user has mistyped the choice of forecast method
...
if forecasting==1
    ...
    if forecast_method==0  % Direct forecasts, we only need to keep the 
        Y = Y1(1:end-1,:);           % last observation for evaluation
        X = X1(1:end-1,:);
        Z = kron(eye(M),X);
        T = T - 1;
    else  % Iterated forecasts, keep the last h observations to evaluate 
        Y = Y1(1:end-h,:);                           % the VAR forecasts
        X = X1(1:end-h,:);
        Z = kron(eye(M),X);
        T = T - h;
    end
...
%             equatN = M; %Interest rate equation
%             impf_m = zeros(M,ihor);
%             jj=0;
%             for ij = 1:ihor
%                 jj = jj + equatN;
